...
## Step 1 — HOST-A: Install Matrix Synapse

On HOST-A, run `launch.sh` but skip everything you already have:
...
"
# Example output: Ex4mPle-P4ss-HostA-ChAng3me!
```

Register the bot with Synapse. On HOST-A, as your desktop user (not root):
...
Also save the bot's Matrix ID: `@host-a:localhost`

---

## Step 3 — HOST-B: Register HOST-B's Hermes Agent in HOST-A's Synapse

On HOST-B, repeat the registration but point at HOST-A's Synapse (using HOST-A's LAN IP).

First, get the registration shared secret from HOST-A:

```bash
# On HOST-A — get the reg secret
sudo grep "registration_shared_secret" /etc/matrix-synapse/homeserver.yaml | grep -oP '(?<=")[^"]+'
```

Copy that secret. On HOST-B, use it to register their bot:

```bash
# On HOST-B — install synapse tools for the registration command
# (or use the same Python approach directly against the API — see below)

# Generate a password on HOST-B
python3 -c "
import secrets, string
chars = string.ascii_letters + string.digits + '!@#\$%^&*'
print(''.join(secrets.choice(chars) for _ in range(28)))
"
# Example output: Ex4mPle-P4ss-HostB-ChAng3me!
```

**Alternative: Register via the admin API directly (no sudo needed)**

On HOST-B, you can register the remote bot via the Synapse admin API using Python:

```python
# Run on HOST-B to register @host-b:localhost via HOST-A's admin API
import json, urllib.request, urllib.error

SYNAPSE_HOST = "http://10.0.0.10:8008"   # HOST-A's LAN IP
ADMIN_USER   = "admin"
ADMIN_PASS   = "changeme"                   # HOST-A's admin password
BOT_NAME     = "host-b"
BOT_PASS     = "Ex4mPle-P4ss-HostB-ChAng3me!"  # password you generated above
...
**HOST-A — update the existing profile's .env:**
...
MATRIX_USER_ID=@host-a:localhost
MATRIX_PASSWORD=Ex4mPle-P4ss-HostA-ChAng3me!
MATRIX_ALLOWED_USERS=@admin:localhost,@host-b:localhost
EOF
```

**HOST-B — update its profile's .env to point at HOST-A's Synapse:**
...
# MATRIX INTEGRATION
# =============================================================================
MATRIX_HOMESERVER=http://10.0.0.10:8008
MATRIX_USER_ID=@host-b:localhost
MATRIX_PASSWORD=Ex4mPle-P4ss-HostB-ChAng3me!
MATRIX_ALLOWED_USERS=@admin:localhost,@host-a:localhost
EOF
```
...
## Step 5 — Both VMs: Verify the Coordination Rooms Exist on HOST-A

On HOST-A, get the room IDs for the 5 coordination rooms (needed for the next step):

```bash
# On HOST-A — read room IDs from the credentials file
...
```bash
# On HOST-A — create all 5 rooms as admin
python3 << 'PYEOF'
import json, urllib.request, urllib.error
...
On HOST-A, join `host-a` to all 5 rooms using the admin API (this bypasses invite requirements):

```python
# On HOST-A — run as your desktop user (not root)
python3 << 'PYEOF'
import json, urllib.request, urllib.error
...
On HOST-B, do the same but for `@host-b:localhost` against `http://10.0.0.10:8008`:

```python
# On HOST-B — join @host-b to all 5 rooms on HOST-A's Synapse
python3 << 'PYEOF'
import json, urllib.request, urllib.error

SYNAPSE  = "http://10.0.0.10:8008"
CREDS    = "/root/Downloads/matrix_credentials.env"
ADMIN    = "admin"
APASS    = "changeme"
BOT      = "@host-b:localhost"
...
```bash
# On HOST-A — restart the gateway (use the correct service name for your setup)
systemctl --user restart hermes-gateway

# On HOST-B — restart its gateway (the service name depends on how it was started)
systemctl --user restart hermes-gateway
...
From Element Desktop (connected to `http://10.0.0.10:8008` as `@admin`), send a message to `@host-a:localhost` and `@host-b:localhost`. If the `MATRIX_ALLOWED_USERS` is set correctly, they will respond.
...
## Summary of Credentials (examples only — always generate your own)

| VM | Bot Matrix ID | Password | Synapse |
|---|---|---|---|
| HOST-A | `@host-a:localhost` | `Ex4mPle-P4ss-HostA-ChAng3me!` | `http://127.0.0.1:8008` |
| HOST-B | `@host-b:localhost` | `Ex4mPle-P4ss-HostB-ChAng3me!` | `http://10.0.0.10:8008` |
...
1. Register the new bot with HOST-A's Synapse via the admin API
...
When you add a new bot, update all existing bots' `.env` to include the new bot in `MATRIX_ALLOWED_USERS`. For example, when adding `hermes-c` on VM-C, update both HOST-A and HOST-B's `.env` files:

```
MATRIX_ALLOWED_USERS=@admin:localhost,@host-a:localhost,@host-b:localhost
```
